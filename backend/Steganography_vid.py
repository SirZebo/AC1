...
def encode_video_with_audio(secret_file, cover_video_path, bits_per_pixel):
            ...
            for j in range(frame.shape[1]):
                if data_index + bits_per_pixel <= data_len:
                    ...
                    for k in range(3):  # for each color channel
                        ...
                        pixel[k] = int(new_pixel, 2) % 256  # Clamping to 0-255
    ...
    # Reattach the audio if it exists
    tmp_audio = extract_audio(cover_video_path)
    if os.path.exists(tmp_audio):
        final_video_path = "stego_video_with_audio.mp4"
        subprocess.call(['ffmpeg', '-i', output_video_path, '-i', tmp_audio, '-c:v', 'copy', '-c:a', 'aac', '-strict', 'experimental', final_video_path, '-y'])
...
decode_video("stego_video.mp4", 2)